...
`,t=t.substring(m.length+1),u=!0),!u){const B=new RegExp(`^ {0,${Math.min(3,l-1)}}(?:[*+-]|\\d{1,9}[.)])`);for(;t&&(f=t.split(`
`,1)[0],v=f,this.options.pedantic&&(v=v.replace(/^ {1,4}(?=( {4})*[^ ])/g,"  ")),!B.test(v));){if(v.search(/[^ ]/)>=l||!v.trim())w+=`
`+v.slice(l);else{if(c)break;w+=`
`+v}c||v.trim()||(c=!0),n+=f+`
`,t=t.substring(f.length+1)}}$.loose||(k?$.loose=!0:/\n *\n *$/.test(n)&&(k=!0)),this.options.gfm&&(i=/^\[[ xX]\] /.exec(w),i&&(o=i[0]!=="[ ] ",w=w.replace(/^\[[ xX]\] +/,""))),$.items.push({type:"list_item",raw:n,task:!!i,checked:o,loose:!1,text:w}),$.raw+=n}$.items[$.items.length-1].raw=n.trimRight(),$.items[$.items.length-1].text=w.trimRight(),$.raw=$.raw.trimRight();const F=$.items.length;for(h=0;h<F;h++){this.lexer.state.top=!1,$.items[h].tokens=this.lexer.blockTokens($.items[h].text,[]);const B=$.items[h].tokens.filter(O=>O.type==="space"),J=B.every(O=>{const tt=O.raw.split("");let Y=0;for(const U of tt)if(U===`
`&&(Y+=1),Y>1)return!0;return!1});!$.loose&&B.length&&J&&($.loose=!0,$.items[h].loose=!0)}return $}}html(t){const e=this.rules.block.html.exec(t);if(e){const n={type:"html",raw:e[0],pre:!this.options.sanitizer&&(e[1]==="pre"||e[1]==="script"||e[1]==="style"),text:e[0]};return this.options.sanitize&&(n.type="paragraph",n.text=this.options.sanitizer?this.options.sanitizer(e[0]):dt(e[0]),n.tokens=[],this.lexer.inline(n.text,n.tokens)),n}}def(t){const e=this.rules.block.def.exec(t);if(e)return e[3]&&(e[3]=e[3].substring(1,e[3].length-1)),{type:"def",tag:e[1].toLowerCase().replace(/\s+/g," "),raw:e[0],href:e[2],title:e[3]}}table(t){const e=this.rules.block.table.exec(t);if(e){const n={type:"table",header:sn(e[1]).map(i=>({text:i})),align:e[2].replace(/^ *|\| *$/g,"").split(/ *\| */),rows:e[3]&&e[3].trim()?e[3].replace(/\n[ \t]*$/,"").split(`
